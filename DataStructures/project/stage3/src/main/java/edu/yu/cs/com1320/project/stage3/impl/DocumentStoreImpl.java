...
public class DocumentStoreImpl implements edu.yu.cs.com1320.project.stage3.DocumentStore {
    ...
    public Set<URI> deleteAll(String keyword) {
        Set<Document> documents = this.trie.deleteAll(keyword);
    ...
    public Set<URI> deleteAllWithPrefix(String keywordPrefix) {
        Set<Document> documents = this.trie.deleteAllWithPrefix(keywordPrefix);
        Set<URI> uris = new HashSet<>();
        if(documents.size() > 1){
            CommandSet<URI> commandSet = new CommandSet<>();
            for(Document d: documents){
